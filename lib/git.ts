 ...
 * Handles GitHub 403/429 rate limit responses specifically.
 */
async function withRetry<T>(fn: () => Promise<T>, retries = 3, delayMs = 1000): Promise<T> {
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            const waitMs = retryAfter ? parseInt(retryAfter) * 1000 : delayMs * Math.pow(2, attempt);
            console.warn(`[GitHub] Rate limited (attempt ${attempt + 1}/${retries + 1}). Waiting ${waitMs}ms...`);
...
export async function fetchRepoTree(repoUrl: string, filter: boolean = true): Promise<GitFileNode[]> {
    ...
    try {
        ...
        const { data: repoData } = await withRetry(() => octokit.request("GET /repos/{owner}/{repo}", {
            owner,
            repo,
        }));
        ...
        const { data: treeData } = await withRetry(() => octokit.request("GET /repos/{owner}/{repo}/git/trees/{tree_sha}", {
            owner,
            repo,
...
export async function fetchFileContent(repoUrl: string, filePath: string, ref?: string): Promise<string> {
    const { owner, repo } = parseGithubRepo(repoUrl);
    ...
    try {
        ...
        const { data } = await withRetry(() => octokit.request("GET /repos/{owner}/{repo}/contents/{path}", params));
        ...
        console.error(`Error fetching file content for ${filePath} at ref ${ref || "latest"}:`, error);
        return "";
    }
}
...
export async function fetchCommitHistory(repoUrl: string, limit: number = 20): Promise<CommitInfo[]> {
    ...
    try {
        const { data } = await withRetry(() => octokit.request("GET /repos/{owner}/{repo}/commits", {
            owner,
            repo,
...
export async function fetchCommitDetails(repoUrl: string, sha: string): Promise<CommitDetails | null> {
    const { owner, repo } = parseGithubRepo(repoUrl);
    try {
        const { data } = await withRetry(() => octokit.request("GET /repos/{owner}/{repo}/commits/{ref}", {
            owner,
            repo,
            ref: sha
        }));
...
export async function fetchRepoStats(repoUrl: string): Promise<RepoStats | null> {
    const { owner, repo } = parseGithubRepo(repoUrl);
    try {
        const { data } = await withRetry(() => octokit.request("GET /repos/{owner}/{repo}", {
            owner,
            repo,
        }));
...
export async function searchCode(repoUrl: string, query: string): Promise<string[]> {
    const { owner, repo } = parseGithubRepo(repoUrl);
    try {
        const { data } = await withRetry(() => octokit.request("GET /search/code", {
